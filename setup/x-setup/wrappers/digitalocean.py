...
def builder(vm_count):
    ...
    pat_path = '/home/steve/.pat/do_key'        # FIXME Parameter hard-coded to expedite testing.
    ...
    if vm_count == 1:
        ...
        payload['names'] = ['{hostname} {n}'.format(hostname=hostname,
                                                    n=n).replace(' ', '-')
                            for n in range(vm_count)]
    payload['region'] = 'nyc1'
    payload['size'] = '1gb'
    ...
    end_state = 'curl -X POST "{endpoint}"           \
                -d \'{payload}\'                     \
                -H "{a_header}"                      \
                -H "{c_header}"'                     \
    ...
    return re.sub(' +', ' ', end_state)     # TODO 2
